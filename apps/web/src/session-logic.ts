...
function toDerivedWorkLogEntry(
  ...
  if (diagnostic) {
    const explicitDiagnosticDetail = asTrimmedString(payload?.detail);
    const diagnosticDetail =
      explicitDiagnosticDetail === null &&
      diagnostic.technicalDetail !== null &&
      normalizePreviewForComparison(diagnostic.preview) !==
        normalizePreviewForComparison(activity.summary)
        ? diagnostic.technicalDetail
        : diagnostic.preview;
    if (
      normalizePreviewForComparison(diagnosticDetail) !==
      normalizePreviewForComparison(activity.summary)
    ) {
