...
/** Cache completed fenced-code prefixes while preserving the full remark pipeline. */
function createIncrementalMarkdownParser(parse: Parser): Parser {
  let cached: ParsedPrefix | undefined;

  return (source, file) => {
    // CR can become half of CRLF, and a suffix BOM may be stripped by a new parser.
    ...
    if (prefix) {
      ...
      // Remark transforms mutate their input. Each render gets a pristine copy.
...
/** Use one parser cache per streaming renderer. */
