 ...
 * pooled per provider. Countdowns update on minute boundaries.
 */
export function UsageLimitsSection({
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      setClockNow(current);
      timeout = setTimeout(tick, Math.max(1, 60_000 - (current % 60_000)));
    };
    timeout = setTimeout(tick, Math.max(1, 60_000 - (now % 60_000)));
    return () => {
  ...
  return <UsageLimitsPooled presentations={selected} now={clockNow} />;
}
