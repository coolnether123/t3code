...
import { deriveActiveWorkerWait } from "../workerToolPresentation";
...
function ChatViewContent(props: ChatViewProps) {
  ...
  const logicalProjectEnvironments = useMemo(() => {
    ...
    const envs: Array<import("./BranchToolbar.logic").EnvironmentOption> = [];
  ...
  const onCompactContext = async () => {
    ...
    if (
      ...
      (context.selectedProvider !== "claude" && context.selectedProvider !== "opencode")
    ) {
      return;
    }

    // The command has its own message. The composer draft and attachments stay untouched.
  ...
  return (
    <div className="relative flex min-h-0 min-w-0 flex-1 overflow-hidden bg-background">
      ...
      {shouldUseRightPanelSheet && rightPanelOpen && activeThreadRef ? (
        <RightPanelSheet open animationDurationMs={200} onClose={closePreviewPanel}>
          <RightPanelTabs
            mode="sheet"
