...
describe("MessagesTimeline", () => {
  it("renders Worker tool calls as friendly cards with advanced details hidden", () => {
    const markup = renderToStaticMarkup(
      <FriendlyWorkerToolCallRow
        call={{
          ...
          durationMs: 1_200,
          rawData: { secret: "should stay behind details" },
        }}
      />,
    );

    expect(markup).toContain("Started Scout");
    expect(markup).toContain("Scan the provider boundary");
    expect(markup).toContain("Advanced");
    expect(markup).not.toContain("should stay behind details");
  });

  it("renders the edit-from-here action for a checkpointed user message", () => {
    const message = buildUserTimelineEntry("Revise this task");
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...buildProps()}
        revertTurnCountByUserMessageId={new Map([[message.message.id, 1]])}
        onEditUserMessage={() => {}}
        canonicalEditMessageIdByTimelineMessageId={
          new Map([[message.message.id, message.message.id]])
        }
  ...
  it("describes an active Worker wait without hiding the regular working row", () => {
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...buildProps()}
        isWorking
    ...
    expect(markup).toContain("Waiting on Scout");
