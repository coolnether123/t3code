...
export const WorkerStartTool = mutatingTool(
  Tool.make("worker_start", {
    description:
      "Start one persistent T3 Worker for a bounded assignment. The Worker does not inherit the parent conversation. State the task once in assignment; title is a label, and context and instructions should contain only non-duplicative supporting material. Set createWorktree to true to give the Worker a separate branch and checkout rooted at cwd. Worker status includes checkout state and keeps the checkout path for follow-ups. Use worker_interrupt to cancel a Worker while its checkout is being created. Do not ask it to report Worker IDs, token usage, or tool counts; T3 reports that server-owned telemetry through Worker tool results. Its runtime access mode inherits the parent session and cannot be overridden in this call. Omit modelSelection to inherit the parent's exact provider instance, model, and options. A standard Codex Worker may select another exact model slug from that provider instance with modelSelection.model. Send modelSelection.options to adjust or add provider options. Set backendPreference to codex-desktop when the assignment needs Codex Desktop's native browser or app capabilities; this preserves a separate native child identity and requires the configured Desktop coordinator. Exact native model slugs may be supplied with codex-desktop and are validated by the Desktop host.",
