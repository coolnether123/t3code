...
it.effect("persists Worker identity and exact model options across follow-up activations", () => {
  ...
  let followUpModelSelection: ModelSelection | undefined;
  ...
  return Effect.gen(function* () {
    ...
    yield* service.send({ workerId: started.summary.id, message: "Continue in Chrome." });
    expect(followUpModelSelection).toEqual({
      instanceId: providerInstanceId,
      model: "gpt-5.6-sol",
      options: [{ id: "computerControl", value: "chrome" }],
    });
