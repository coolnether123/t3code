  ...
  const mcpCapabilitiesFor = (
    ...
    serverSettings.getSettings.pipe(
      Effect.map((settings) => {
        ...
        // Browser/computer MCP access is governed by its explicit setting.
        // Workers remain independently available when enabled because their
        // server-side lifecycle is not browser access.
        return settings.enableAgentBrowserAccess
          ? capabilities
          : new Set<McpInvocationContext.McpCapability>(
              capabilities.has("workers") ? ["workers"] : [],
            );
      }),
      Effect.catch((cause) =>
        Effect.logWarning(
