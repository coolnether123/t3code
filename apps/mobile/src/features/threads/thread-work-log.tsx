...
import type { EnvironmentId, ToolActivityIcon, ToolScreenshot } from "@t3tools/contracts";
...
const ThreadWorkLogRow = memo(function ThreadWorkLogRow(
  ...
  const screenshot = expanded ? row.screenshot : undefined;
...
function ToolScreenshotImage(props: {
  ...
  return (
    <View className="mb-2 max-h-80 overflow-hidden rounded-lg">
      {props.renderImage({
        href: asset.url,
        alt: `Chrome screenshot (${props.screenshot.width} × ${props.screenshot.height})`,
        title: null,
      })}
    </View>
  );
